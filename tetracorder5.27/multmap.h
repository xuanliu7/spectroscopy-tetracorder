...
################################################################################
	...
	parameter	(mfilelen=72) # max length of output file names
                                      # if change, must change values below
	parameter	(mpgfilelen=150) # max length of output file names + path + group
                                         # mfilelen + pathgrp length + 6
					 # pathgrp length is currently 72 characters
                                      # if change, must change values below
...
#					  #     for each pixel in the line.
	...
	character*60 matid(maxmat)        # material ID
...
#                 removed shapes.  2 values are used for fuzzy logic.
	...
	# NOTE: maxfilelen was not defined previous to 4/2021.  mfilelen=72 is defined, 
	#			so change maxfilelen to mfilelen
	#       pathgrp is defined below as 72 characters.
	...
	#     if change the constabts here, also change mfilelen and  mpgfilelen parameters above
        character mfile(maxmat)*72        # base file name (*mfilelen)
	character dfile*150               # file names  (*mfilelen+pathgrp+6)
	character ffile*150               # file names  (*mfilelen+pathgrp+6)
	character fdfile*150              # file names  (*mfilelen+pathgrp+6)

	character*72 pathgrp(0:maxgrp)    # group path name NOTE: keep pathgrp and pathcase length the same
	character*72 pathcase(maxcse)     # case  path name NOTE: keep pathgrp and pathcase length the same
